...
def screen0():  # Main Menu
    while True:
        ...
        # Speech Recognition Mode button
        sr_btn = pygame.draw.rect(screen, 'light yellow', [width // 2 - 180, height // 2 + 150, 360, 60], 0, 5)
        pygame.draw.rect(screen, 'blue', [width // 2 - 180, height // 2 + 150, 360, 60], 5, 5)
        text_sr = font.render('Speech Recognition Mode', True, 'black')
        text_sr_rect = text_sr.get_rect(center=sr_btn.center)
        screen.blit(text_sr, text_sr_rect)
        ...
        elif sr_btn.collidepoint(pygame.mouse.get_pos()) and pygame.mouse.get_pressed()[0]:
            return 6  # Speech Recognition Mode option selected
...
def screen6():  # Speech Recognition Single Player Menu
    global previous_screen
    previous_screen = 7
    ai2 = True
    error_message = ""
    ...
    while ai2:
        ...
        menu_btn = pygame.draw.rect(screen, 'light gray', [230, 350, 260, 60], 0, 5)
        pygame.draw.rect(screen, 'dark gray', [230, 350, 260, 60], 5, 5)
        text = font.render('Start game', True, 'black')
        screen.blit(text, (295, 367))

        menu_btn_menu = pygame.draw.rect(screen, 'light gray', [230, 450, 260, 60], 0, 5)
        pygame.draw.rect(screen, 'dark gray', [230, 450, 260, 60], 5, 5)
        text = font.render('Back', True, 'black')
        screen.blit(text, (325, 467))
...
def screen7():  # Single Player Game
    global winner
    global winnercolor
        ...
        for event in pygame.event.get():
            ...
            if not game_over:
                if player1_turn:  # Player 1's turn
                    ...
                    if not computer_move():
                        pygame.time.wait(1000)  # Wait before dropping piece
                        gl.print_board(board)
                        gl.draw_board(board, screen, RADIUS, height, colour_p1, colour_p2)
                        if gl.winning_move(board, 2):
                            turn_count = str(turn_count_p2)  # Update the turn count for the computer
...
while run:
    ...
    if screen_number == 0:
        ...
        screen_number = screen6()
    elif screen_number == 7:
        screen_number = screen7()
    else:
